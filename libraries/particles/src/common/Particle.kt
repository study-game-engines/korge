...
internal const val PARTICLE_STRIDE: Int = 27

@JvmInline
value class Particle(val index: Int) {
    val offset get() = index * PARTICLE_STRIDE
}

open class ParticleContainer(val max: Int) {
    val data: Buffer = Buffer.allocDirect(max * PARTICLE_STRIDE * Float.SIZE_BYTES)
...
inline fun <T : ParticleContainer, R> T.map(max: Int = this.max, block: T.(Particle) -> R): List<R> = (0 until max).map { block(Particle(it)) }
