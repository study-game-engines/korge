...
import korlibs.graphics.gl.versionInt
...
class KmlGlJsCanvas(val canvas: HTMLCanvasElement, val glOpts: dynamic) : KmlGl() {
    ...
    private fun <T> Int.free(): T? = if (this != 0) {
        val out = items[this].unsafeCast<T>(); freeList += this; items[this] = null; out
    } else {
        null
    }
    ...
    override fun compressedTexImage2D(target: Int, level: Int, internalformat: Int, width: Int, height: Int, border: Int, imageSize: Int, data: Buffer): Unit =
        gl.compressedTexImage2D(target, level, internalformat, width, height, border, data.arrayByte)
    ...
    override fun compressedTexSubImage2D(target: Int, level: Int, xoffset: Int, yoffset: Int, width: Int, height: Int, format: Int, imageSize: Int, data: Buffer): Unit =
        gl.compressedTexSubImage2D(target, level, xoffset, yoffset, width, height, format, data.arrayByte)
    ...
    override fun deleteBuffers(n: Int, items: Buffer) {
        for (p in 0 until n) gl.deleteBuffer(items.arrayInt[p].free())
    }

    override fun deleteFramebuffers(n: Int, items: Buffer) {
        for (p in 0 until n) gl.deleteFramebuffer(items.arrayInt[p].free())
    }
    ...
    override fun deleteRenderbuffers(n: Int, items: Buffer) {
        for (p in 0 until n) gl.deleteRenderbuffer(items.arrayInt[p].free())
    }

    override fun deleteShader(shader: Int): Unit = gl.deleteShader(shader.get())
    override fun deleteTextures(n: Int, items: Buffer) {
        for (p in 0 until n) gl.deleteTexture(items.arrayInt[p].free())
    }
    ...
    override fun genBuffers(n: Int, buffers: Buffer) {
        for (p in 0 until n) buffers.arrayInt[p] = gl.createBuffer().alloc()
    }
    ...
    override fun genFramebuffers(n: Int, framebuffers: Buffer) {
        for (p in 0 until n) framebuffers.arrayInt[p] = gl.createFramebuffer().alloc()
    }

    override fun genRenderbuffers(n: Int, renderbuffers: Buffer) {
        for (p in 0 until n) renderbuffers.arrayInt[p] = gl.createRenderbuffer().alloc()
    }

    override fun genTextures(n: Int, textures: Buffer) {
        for (p in 0 until n) textures.arrayInt[p] = gl.createTexture().alloc()
    }

    override fun getActiveAttrib(program: Int, index: Int, bufSize: Int, length: Buffer, size: Buffer, type: Buffer, name: Buffer) {
        val info = gl.getActiveAttrib(program.get(), index)!!; size.arrayInt[0] = info.size; type.arrayInt[0] = info.type; name.putAsciiString(info.name); length.arrayInt[0] = info.size + 1
    }

    override fun getActiveUniform(program: Int, index: Int, bufSize: Int, length: Buffer, size: Buffer, type: Buffer, name: Buffer) {
        val info = gl.getActiveUniform(program.get(), index)!!; size.arrayInt[0] = info.size; type.arrayInt[0] = info.type; name.putAsciiString(info.name); length.arrayInt[0] = info.size + 1
    }

    override fun getAttachedShaders(program: Int, maxCount: Int, count: Buffer, shaders: Buffer) {
        val ashaders = gl.getAttachedShaders(program.get())!!; count.arrayInt[0] = ashaders.size; for (n in 0 until min(maxCount, ashaders.size)) shaders.arrayInt[n] = ashaders[n].asDynamic().id.unsafeCast<Int>()
    }
    ...
    override fun getBooleanv(pname: Int, data: Buffer) {
        data.arrayInt[0] = gl.getParameter(pname).unsafeCast<Int>()
    }

    override fun getBufferParameteriv(target: Int, pname: Int, params: Buffer) {
        params.arrayInt[0] = gl.getBufferParameter(target, pname).unsafeCast<Int>()
    }

    override fun getError(): Int = gl.getError()
    override fun getFloatv(pname: Int, data: Buffer) {
        data.arrayFloat[0] = gl.getParameter(pname).unsafeCast<Float>()
    }

    override fun getFramebufferAttachmentParameteriv(target: Int, attachment: Int, pname: Int, params: Buffer) {
        params.arrayInt[0] = gl.getFramebufferAttachmentParameter(target, attachment, pname).unsafeCast<Int>()
    }
    ...
    override fun getProgramInfoLog(program: Int, bufSize: Int, length: Buffer, infoLog: Buffer) {
        val str = gl.getProgramInfoLog(program.get()) ?: ""; length.arrayInt[0] = str.length; infoLog.putAsciiString(str)
    }

    override fun getRenderbufferParameteriv(target: Int, pname: Int, params: Buffer) {
        params.arrayInt[0] = gl.getRenderbufferParameter(target, pname).unsafeCast<Int>()
    }
    ...
    override fun getShaderInfoLog(shader: Int, bufSize: Int, length: Buffer, infoLog: Buffer) {
        val str = gl.getShaderInfoLog(shader.get()) ?: ""; length.arrayInt[0] = str.length; infoLog.putAsciiString(str)
    }

    override fun getShaderPrecisionFormat(shadertype: Int, precisiontype: Int, range: Buffer, precision: Buffer) {
        val info = gl.getShaderPrecisionFormat(shadertype, precisiontype); if (info != null) {
            range.arrayInt[0] = info.rangeMin; range.arrayInt[1] = info.rangeMax; precision.arrayInt[0] = info.precision
        }
    }

    override fun getShaderSource(shader: Int, bufSize: Int, length: Buffer, source: Buffer) {
        val str = gl.getShaderSource(shader.get()) ?: ""; length.arrayInt[0] = str.length; source.putAsciiString(str)
    }
    ...
    override fun getTexParameterfv(target: Int, pname: Int, params: Buffer) {
        params.arrayFloat[0] = gl.getTexParameter(target, pname).unsafeCast<Float>()
    }

    override fun getTexParameteriv(target: Int, pname: Int, params: Buffer) {
        params.arrayInt[0] = gl.getTexParameter(target, pname).unsafeCast<Int>()
    }

    override fun getUniformfv(program: Int, location: Int, params: Buffer) {
        params.arrayFloat[0] = gl.getUniform(program.get(), location.get()).unsafeCast<Float>()
    }

    override fun getUniformiv(program: Int, location: Int, params: Buffer) {
        params.arrayInt[0] = gl.getUniform(program.get(), location.get()).unsafeCast<Int>()
    }

    override fun getVertexAttribfv(index: Int, pname: Int, params: Buffer) {
        params.arrayFloat[0] = gl.getVertexAttrib(index, pname).unsafeCast<Float>()
    }

    override fun getVertexAttribiv(index: Int, pname: Int, params: Buffer) {
        params.arrayInt[0] = gl.getVertexAttrib(index, pname).unsafeCast<Int>()
    }

    override fun getVertexAttribPointerv(index: Int, pname: Int, pointer: Buffer) {
        pointer.arrayInt[0] = gl.getVertexAttrib(index, pname).unsafeCast<Int>()
    }
    ...
    override fun texSubImage2D(target: Int, level: Int, xoffset: Int, yoffset: Int, width: Int, height: Int, format: Int, type: Int, pixels: Buffer): Unit =
        gl.texSubImage2D(target, level, xoffset, yoffset, width, height, format, type, pixels.arrayUByte)
    ...
    override val isFloatTextureSupported: Boolean by lazy {
        gl.getExtension("OES_texture_float_linear") != null // Also request it to support linear filtering if possible!
                || gl.getExtension("OES_texture_float") != null || webglVersion >= 2 // Supported by default in WebGL 2
    }

    val instancedArrays: WebGLExtension = WebGLExtension(this, "ANGLE_instanced_arrays", coreSince = 2, functions = listOf("drawArraysInstanced", "drawElementsInstanced", "vertexAttribDivisor"), suffix = "ANGLE")
