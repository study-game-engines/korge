...
fun ProgramWithDefault(vertex: VertexShader = DefaultShaders.VERTEX_DEFAULT, fragment: FragmentShader = DefaultShaders.FRAGMENT_SOLID_COLOR, name: String = "program"): Program = Program(vertex, fragment, name)
...
inline fun VertexShaderDefault(callback: ProgramBuilderDefault.() -> Unit): VertexShader = VertexShader(ProgramBuilderDefault().also(callback)._buildFuncs())

inline fun FragmentShaderDefault(callback: ProgramBuilderDefault.() -> Unit): FragmentShader = FragmentShader(ProgramBuilderDefault().also(callback)._buildFuncs())
