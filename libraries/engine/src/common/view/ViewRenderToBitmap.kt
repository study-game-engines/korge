...
suspend fun View.renderToBitmap(views: Views? = this.stage?.views, region: Rectangle? = null, scale: Double = 1.0, outPoint: Ref<Point>? = null, includeBackground: Boolean = false): Bitmap32 {
	...
	val done: CompletableDeferred<Bitmap32> = CompletableDeferred()
    views.stage.invalidateRender()
    views.onBeforeRender.once { renderContext ->
        done.completeWith(runCatching {
            unsafeRenderToBitmapSync(renderContext, region, scale, outPoint, bgcolor = if (includeBackground) views.clearColor else Colors.TRANSPARENT)
        })
    }
    return done.await()
}
...
fun View.unsafeRenderToBitmapSync(ctx: RenderContext, region: Rectangle? = null, scale: Double = 1.0, outPoint: Ref<Point>? = null, useTexture: Boolean = true, bgcolor: RGBA = Colors.TRANSPARENT): Bitmap32 {
    ...
    return Bitmap32((region?.width ?: bounds.width).toInt(), (region?.height ?: bounds.height).toInt(), premultiplied = true).also { bmp ->
