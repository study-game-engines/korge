...
open class Animator @PublishedApi internal constructor(
    ...
    private fun ensure() {
        ...
        rootAnimator.updater = this.root.addFastUpdater(first = rootAnimator.startImmediately) { dt ->
    ...
    inner class RootAnimationNode : NewAnimatorNode {
        ...
        override fun update(dt: FastDuration): FastDuration {
            ...
            ensureInit()
            if (parallel) {
                var completedTime: FastDuration = 0.fastSeconds
                toRemove.clear()
...
fun Animator.block(name: String? = null, callback: () -> Unit) = addNode(Animator.BlockNode(name, callback))
fun Animator.removeFromParent(view: View) = block { view.removeFromParent() }
...
fun Animator.scaleTo(view: View, scaleX: () -> Number, scaleY: () -> Number = scaleX, time: Duration = this.defaultTime, lazyTime: (() -> Duration)? = null, easing: Easing = this.defaultEasing) = __tween({ view::scaleX[scaleX()] }, { view::scaleY[scaleY()] }, time = time.fast, lazyTime = lazyTime.fast, easing = easing, name = "scaleTo")
...
fun Animator.moveTo(view: View, x: () -> Number = { view.x }, y: () -> Number = { view.y }, time: Duration = this.defaultTime, lazyTime: (() -> Duration)? = null, easing: Easing = this.defaultEasing) = __tween({ view::x[x()] }, { view::y[y()] }, time = time.fast, lazyTime = lazyTime.fast, easing = easing, name = "moveTo")
...
fun Animator.moveInPath(view: View, path: VectorPath, includeLastPoint: Boolean = true, time: Duration = this.defaultTime, lazyTime: (() -> Duration)? = null, easing: Easing = this.defaultEasing) = __tween({ view::pos.get(path, includeLastPoint = includeLastPoint) }, time = time.fast, lazyTime = lazyTime.fast, easing = easing, name = "moveInPath")
fun Animator.moveInPath(view: View, points: PointList, time: Duration = this.defaultTime, lazyTime: (() -> Duration)? = null, easing: Easing = this.defaultEasing) = __tween({ view::pos[points] }, time = time.fast, lazyTime = lazyTime.fast, easing = easing, name = "moveInPath")
...
fun Animator.rotateTo(view: View, rotation: () -> Angle, time: Duration = this.defaultTime, lazyTime: (() -> Duration)? = null, easing: Easing = this.defaultEasing) = __tween({ view::rotation[rotation()] }, time = time.fast, lazyTime = lazyTime.fast, easing = easing, name = "rotateTo")
